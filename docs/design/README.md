...
| "Generate Order Link" | ✅ Built — a dialog on the same button (Figma draws no destination for it), mirroring the outgoing React app's `OrderLinkSection` and the API's `customers.tokens` endpoints exactly. The link's destination is ✅ built too, as a new Vue page (`PublicOrder.vue`, no Figma frame exists for it) rather than a port of `frontend/src/app/order/[token]` — an unauthenticated catalog + order form that talks directly to the existing public JSON API. Building it surfaced a real pre-existing bug in that API: `unit_type` defaulted off the customer's `allow_single_bottle` flag alone, which could pick a unit the specific item doesn't sell in and get rejected; fixed by defaulting to each item's own `sales_unit` instead, and excluding bottles-only items from a cases-only customer's catalog entirely |
