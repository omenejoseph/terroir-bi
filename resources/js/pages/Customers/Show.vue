...
const ORDER_HISTORY_RANGE_TABS: TabItem[] = [
...
function orderStatusLabel(status: string): string {
    return props.orderStatusCounts?.statuses.find((s) => s.key === status)?.label ?? ORDER_STATUS_LABEL[status] ?? status;
}
 ...
 */
const ORDER_FLAG: Partial<Record<OrderStatusKey, string>> = {
    IN_PROCESS: 'Awaiting fulfilment',
    READY_TO_SHIP: 'Ready — schedule delivery',
};

function orderFlag(order: Order): string | null {
    return ORDER_FLAG[order.status] ?? null;
}
...
function removePrice(row: CustomerPriceRow): void {
    if (!confirm(`Remove the price set for ${row.name}? They will pay the tier/list price instead.`)) return;
                        ...
                        <SectionHeader
                            ...
                            :description="
                                customer.pricing_tier
                                    ? `Tier ${customer.pricing_tier.name} · ${customer.effective_rebate_percent}% rebate applies to everything else`
                                    : `No tier · ${customer.effective_rebate_percent}% rebate applies to everything else`
                            "
                        />
                            ...
                            <tbody>
                                    ...
                                    <td v-if="can('pricing.manage')" class="px-6 py-3">
                                        <div class="flex items-center justify-end gap-1">
                                            ...
                                            <button
                                                ...
                                                :aria-label="`Remove price for ${row.name}`"
                                ...
                                <tr v-if="(pricing?.rows ?? []).length === 0">
                                    <td :colspan="can('pricing.manage') ? 4 : 3" class="px-6 py-12 text-center text-muted-foreground">
                                        No negotiated prices yet — this customer pays
                                        {{ customer.pricing_tier ? `the ${customer.pricing_tier.name} tier` : 'list' }}
                                        price on everything.
                                    </td>
                                </tr>
                            </tbody>
            ...
            <template v-else-if="tab === 'orders'">
                ...
                <div class="overflow-hidden border border-border bg-card">
                    ...
                    <div class="flex flex-wrap items-center gap-2 border-b border-border px-4 py-3">
                        <div class="relative w-full max-w-[280px]">
                            ...
                            <input
                                ...
                                aria-label="Search this customer's orders"
                        ...
                        <table class="w-full min-w-[52rem] text-xs">
                            <thead class="border-b border-border bg-muted/40 text-left text-3xs text-muted-foreground">
                                <tr>
                                    ...
                                    <th scope="col" class="px-4 py-2.5 text-right font-medium tracking-[0.08em] uppercase">Total</th>
                                    <th scope="col" class="px-4 py-2.5 font-medium tracking-[0.08em] uppercase">Status</th>
                                    <th scope="col" class="px-4 py-2.5 font-medium tracking-[0.08em] uppercase">Flag</th>
                                    <th scope="col" class="px-4 py-2.5 text-right font-medium tracking-[0.08em] uppercase">Date</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-3 border-t border-border px-4 py-3">
                        <p class="text-xs text-muted-foreground">
                            {{ orderHistoryShowing }}
                            <template v-if="orderHistoryTotal"> · Total {{ money(orderHistoryTotal) }}</template>
                        </p>
