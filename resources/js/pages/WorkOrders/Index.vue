 ...
 *
 * Two structural notes, both explained in App\Services\Tasks\WorkOrderBoard.
 * The design's named boards do not exist as a domain concept, so the picker
 * lists the categories that have work. And the design's four lists include
 * "This Week", which is a date window rather than a status — dragging a card
 * into it would have to mean "reschedule", so the columns are the three real
 * statuses and "Due soon" answers the same question as a filter.
 */
...
function selectBoard(key: string | null): void {
    reload({ category: props.filters.category === key ? undefined : (key ?? undefined) });
}
        ...
        <div class="space-y-4">
            <PageHeader title="Work Orders" :description="summary">
                <template #actions>
                    ...
                    <!-- @todo Favourites and New Board. Both need a board entity
                         this domain does not have; the picker below is built out
                         of categories instead. -->
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <button
                    ...
                    :key="option.key"
                    type="button"
                    :aria-pressed="filters.category === option.key"
                    :class="
                        cn(
                            'inline-flex h-8 items-center gap-2 px-3 text-xs transition-colors',
            ...
            <p v-if="board.total === 0" class="border border-border bg-card px-4 py-12 text-center text-xs text-muted-foreground">
                {{
                    filters.search || filters.category || filters.due_soon || filters.mine
